...
// ==================== UPLOAD PORTFOLIO - IMAGEM DE CAPA ====================
...
// ==================== UPLOAD PORTFOLIO - MÚLTIPLAS IMAGENS MOBILE ====================
export async function uploadPortfolioMobileImages(files, itemId) {
  try {
    console.log(`📤 Iniciando upload de ${files.length} imagens mobile`)
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (uploadError) {
        console.error(`❌ Erro ao fazer upload da imagem ${i}:`, uploadError)
        throw uploadError
      }
      ...
      urls.push(publicUrl)
      console.log(`✅ Imagem mobile ${i + 1}/${files.length} enviada`)
    }
    
    console.log('✅ Todas as imagens mobile foram enviadas')
    return urls
  } catch (error) {
    console.error('❌ Erro ao fazer upload das imagens mobile:', error)
    throw error
  }
}

// ==================== UPLOAD PORTFOLIO - MÚLTIPLAS IMAGENS DESKTOP ====================
export async function uploadPortfolioDesktopImages(files, itemId) {
  try {
    console.log(`📤 Iniciando upload de ${files.length} imagens desktop`)
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      urls.push(publicUrl)
      console.log(`✅ Imagem desktop ${i + 1}/${files.length} enviada`)
    }
    
    console.log('✅ Todas as imagens desktop foram enviadas')
    return urls
  } catch (error) {
    console.error('❌ Erro ao fazer upload das imagens desktop:', error)
    throw error
  }
}

// ==================== DELETE PORTFOLIO - MÚLTIPLAS IMAGENS ====================
export async function deletePortfolioImages(urls) {
  try {
    ...
    console.log(`🗑️ Deletando ${paths.length} imagens`)
    ...
    console.error('❌ Erro ao deletar imagens do portfólio:', error)
  }
}